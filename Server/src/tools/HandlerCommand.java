...
//        Stream stream=collection.entrySet().stream();
//        stream.sorted(comparingByValue()).forEach(x-> message.append("\n").append(x).toString());
//        send(message.toString());

//        String send="";
//        for (Map.Entry<Integer, Ticket> element : collection.values()) {
//            if(send.length()==0){
...
//            }
//        }
//        send=send+"\n"+"Все элементы коллекции успешно выведены";
//        send(send);
    }
...
//        }
    ...
    }else {send("Элементы с указанным id не существует");}
    }
...
//       send(send);
    ...
    public void commandsRemoveLowerKey(int key) {
        ...
        send("Элементы id которых меньше чем "+key+" и на которые у вас есть права  успешно удалены");
...
//        collection.entrySet().stream().map(Map.Entry::getValue).filter(x->x.getType()==TicketType.CHEAP).forEach(x->stringBuilder.append("\n").append(x).toString());
        ...
        //Map<Integer, Ticket> sortedMap = collection.entrySet().stream().filter());

//        ArrayList<String> vipTicket = new ArrayList<String>();
//        ArrayList<String> usualTicket = new ArrayList<String>();
//        ArrayList<String> budgetaryTicket = new ArrayList<String>();
//        ArrayList<String> cheapTicket = new ArrayList<String>();
//        for (Map.Entry<Integer, Ticket> element : collection.entrySet()) {
//            Ticket ticket = collection.get(element.getKey());
//            if (ticket.getType() == TicketType.VIP) {
//                vipTicket.add("id билета: " + ticket.getId() + " тип билета VIP");
//            } else if (ticket.getType() == TicketType.USUAL) {
//                usualTicket.add("id билета: " + ticket.getId() + " тип билета USUAL");
//            } else if (ticket.getType() == TicketType.BUDGETARY) {
//                budgetaryTicket.add("id билета: " + ticket.getId() + " тип билета BUDGETARY");
//            } else {
//                cheapTicket.add("id билета: " + ticket.getId() + " тип билета CHEAP");
//            }
//
//        }
//        for (String tiketType : vipTicket) {
//            commandsPrintFieldDescendingTypeMakeString(tiketType);
//        }
//        for (String tiketType : usualTicket) {
//            commandsPrintFieldDescendingTypeMakeString(tiketType);
//        }
//        for (String tiketType : budgetaryTicket) {
//            commandsPrintFieldDescendingTypeMakeString(tiketType);
//        }
//        for (String tiketType : cheapTicket) {
//            commandsPrintFieldDescendingTypeMakeString(tiketType);
//        }
//            send(StringcommandsPrintFieldDescendingTypeMakeString);
    }
...
//        send("Все элементы успешно удалены.");
    ...
    public void commandsUpdateExecute(int key, ArrayList commands) {
        if(collection.get(key)==null){
            send("Обьекта с таким id не существует");
        }else {
        if(!checkAuthor(key)){
    ...
    public void newCommandsExecuteScript(ArrayList<String> arrayList){
        CommandsExecuteScript commandsExecuteScript = new CommandsExecuteScript(arrayList,this);
